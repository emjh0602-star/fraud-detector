...
def analyze_transactions(rows, corp_name, history):
    ...
    hist_amounts = [a for a in (clean_amount(r.get("amount",0)) for r in corp_history) if a>0]
    hist_payees  = {r.get("payee","") for r in corp_history if r.get("payee")}
    ...
    for r in rows:
        ...
        payee = r.get("payee","")
        if payee and hist_payees and payee not in hist_payees:
            reasons.append("신규 수취인"); score+=1
            if amt>=10_000_000: reasons.append("신규 수취인 고액"); score+=1
